...
`POST /api/v1/procedures/{procedure_id}/versions`
...
Bloquea el procedimiento para asignar el siguiente numero. Crea un borrador vacio, no clona la version anterior. Si se especifica source_session_id verifica que sea de la misma organizacion.
...
```json
{
  "summary": "Primera version explicada por el experto"
}
```

## Respuesta

Exito: **201**. `VersionResponse`. Campos y tipos en [contratos](contratos.md). Las listas son arrays JSON sin envoltorio total/items.

## Persistencia y limites

procedures, procedure_versions, learning_sessions opcional, audit_events.
