...
`PUT /api/v1/procedure-versions/{version_id}/steps/{step_id}`
...
PUT reemplaza los campos del paso. Debe pertenecer a esa version. Una posicion ocupada responde 409; una version fuera de draft no se edita.
...
Exito: **200**. `StepResponse`. Campos y tipos en [contratos](contratos.md). Las listas son arrays JSON sin envoltorio total/items.
